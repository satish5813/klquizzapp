...
export interface FacAttendance { open: boolean; posted: boolean; postedAt: string | null; marks: Record<string, boolean> | null; }
...
export interface AttReport { session: { open: boolean; openedAt: string | null; closedAt: string | null }; summary: { faculties: number; posted: number; notPosted: number; totalStudents: number; present: number; absent: number }; sections: AttSection[]; }
